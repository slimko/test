...
<form action="http://{$smarty.server.SERVER_NAME}/post" method="post" class="contact_form">
...
<span style="color:red;">{$price_error|default:''}</span>	
	...
	{foreach from=$bd key=nameValue item=Value }
		<li>{$Value.id}. <a href="http://{$smarty.server.SERVER_NAME}/get/id/{$Value.id}">{$Value.title_ad|escape:htmlall:'UTF-8'}</a> | {$Value.price|escape:htmlall:'UTF-8'} руб. | <a href="http://{$smarty.server.SERVER_NAME}/del/id/{$nameValue}">удалить</a></li>
		{foreachelse}
