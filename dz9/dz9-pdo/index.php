...
if($_POST){
    ...
    if(is_numeric($_POST['price'])){
        if($_POST['id']){ //проверяем наличие id у формы
            $result = array_merge ($form, $_POST);
            updateBD($pdo,$result); //отправляем в базу данных на обновление
        }
        else{
            $post = array_diff($_POST, array(null)); // удалим id с пустым значением
            $result = array_merge ($form, $post);
            postBD($pdo,$result); //отправляем данные в базу данных
        }
...
$params = getDataBD($pdo);//получаем объявления и данные для формы из БД
